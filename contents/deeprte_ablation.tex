...
\end{equation}
其中 $\tau^{\text{NN}}_{-}$ 在算法~\ref{alg:code-optical-depth-net} 中定义。$\bG^{\text{NN}}$ 的输出维度为 $d_\text{model}$，$\bm{W} \in \mathbb{R}^{d_{\text{model}}\times 1}$ 是一个可学习的权重矩阵，用于将输出投影到单个输出通道，类似于~\eqref{scattering_layer} 中的 $\bm{W}$。
