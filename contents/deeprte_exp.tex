\section {通用数值实验设定}
\label {sec:common-experiments}

本节介绍后续数值实验中通用的物理模型约化、计算域离散、相函数选择、截面参数设置以及基础训练配置。这些设定为 DeepRTE 与 EfficientRTE 的性能评估提供了统一的基准平台。

\subsection{三维问题的二维约化}

实际应用中，含源项的辐射传输方程（radiative transfer equation, RTE）定义在三维空间变量 $\boldsymbol{r}\in\mathbb{R}^3$ 与角变量 $\boldsymbol{\Omega}\in\mathbb{S}^2$ 上，对应 $d=3$，是一个六维问题。借助适当的变量变换与对称性分析，可将其约化为低维模型，从而降低计算复杂度。

在笛卡尔坐标系下，首先写出含体源项 $q(\boldsymbol{r},\boldsymbol{\Omega})$ 的三维稳态RTE完整形式：
\begin{equation}\label{eq:rte-3d-source}
    \begin{aligned}
        (\boldsymbol{\Omega}\cdot\nabla + \mu_t(\boldsymbol{r})) I(\boldsymbol{r},\boldsymbol{\Omega})
        = \frac{\mu_s(\boldsymbol{r})}{4\pi} \int_{\mathbb{S}^2} p(\boldsymbol{\Omega},\boldsymbol{\Omega}^*) I(\boldsymbol{r},\boldsymbol{\Omega}^*) \,\mathrm{d}\boldsymbol{\Omega}^*
        + q(\boldsymbol{r},\boldsymbol{\Omega}),
    \end{aligned}
\end{equation}
其中 $q(\boldsymbol{r},\boldsymbol{\Omega})$ 为相空间内的体源项，其余符号含义与无源项情形一致。

对三维角度向量 $\boldsymbol{\Omega}$ 做参数化，记
\begin{equation}
    \boldsymbol{\Omega}=(c,s,\zeta), \quad c =
        {\left(1-\zeta^{2}\right)}^{\frac{1}{2}} \cos\theta, \quad s =
        {\left(1-\zeta^{2}\right)}^{\frac{1}{2}} \sin\theta, \quad \text{for }|\zeta| \leq 1,
\end{equation}
其中 $\zeta\in[-1,1]$ 为极角余弦，$\theta\in[0,2\pi)$ 为方位角，此时角度微元满足 $\mathrm{d}\boldsymbol{\Omega} = \mathrm{d}\zeta \mathrm{d}\theta$。

若总截面 $\mu_t$ 与散射截面 $\mu_s$ 仅依赖于平面坐标 $(x,y)$，强度 $I$ 在 $z$ 方向上均匀，且体源项 $q$ 同样在 $z$ 方向上均匀，我们可定义关于 $\zeta$ 的偶对称平均，将所有物理量约化为二维平面上的量。具体而言，定义约化辐射强度
\begin{equation}
...
以及约化体源项
\begin{equation}
    \tilde{q}(x, y,\zeta,\theta) = \frac{1}{2}\bigl[q(x,y,z,c,s,\zeta) + q(x,y,z,c,s,-\zeta)\bigr].
\end{equation}
此时 $\tilde{I}$、$\tilde{p}$ 与 $\tilde{q}$ 均与 $z$ 无关，且关于 $\zeta$ 为偶函数。

将上述约化量代入三维方程~\eqref{eq:rte-3d-source}，利用梯度算子的分解 $\nabla = (\partial_x, \partial_y, \partial_z)$ 以及 $\partial_z I = 0$、$\partial_z q = 0$ 的均匀性假设，对流项可简化为 $c\partial_x \tilde{I} + s\partial_y \tilde{I}$。对于散射项，三维积分 $\int_{\mathbb{S}^2} \cdot \mathrm{d}\boldsymbol{\Omega}^*$ 可分解为 $\int_0^{2\pi} \int_{-1}^1 \cdot \mathrm{d}\zeta^* \mathrm{d}\theta^*$，利用约化量关于 $\zeta$ 的偶对称性，可进一步将积分范围约化为 $[0,1]$ 并乘以因子2，即 $2\int_0^{2\pi} \int_0^1 \cdot \mathrm{d}\zeta^* \mathrm{d}\theta^*$，该因子与相函数原有的归一化因子 $1/(4\pi)$ 结合后得到 $1/(2\pi)$。

经过上述推导，含源项的三维RTE最终可约化为如下二维平面问题：
\begin{equation}\label{eq:rte-2d-reduced-source}
    \begin{aligned}
         & c\partial_x \tilde{I}(x,y,\zeta,\theta)+s\partial_y \tilde{I}(x,y,\zeta,\theta)+\mu_t \,\tilde{I}(x,y,\zeta,\theta)                                                       \\
         & \quad= \frac{\mu_s}{2\pi}\int_{0}^{1}\int_0^{2\pi} \tilde{p}(\zeta, \theta, \zeta^*,\theta^*) \,\tilde{I}(x,y,\zeta^*,\theta^*) \,\mathrm{d}{\theta^*}\mathrm{d}{\zeta^*}
        + \tilde{q}(x,y,\zeta,\theta),
    \end{aligned}
\end{equation}
对应的入射边界条件为
\begin{equation}
    \tilde{I}(x,y,\boldsymbol{\Omega})=\tilde{I}_{-}(x,y,\boldsymbol{\Omega}), \quad \text{当 }
    \boldsymbol{\Omega}\cdot\bm{n}<0,\quad (x, y)\in\partial D.
\end{equation}

后续数值实验均基于该含源项的二维约化模型进行。为简化记号，下文中将省略约化量的上标，直接用 $I$、$p$、$q$ 表示二维约化后的物理量。

相函数 $p$ 是辐射传输方程的核心组成，通常仅与入射、出射方向的夹角相关。本文选用归一化的 Henyey--Greenstein（H-G）相函数作为各向异性散射的代表模型：
\begin{equation}
    p(\bOmega,\bOmega^*) = p(\bOmega\cdot\bOmega^*) = \frac{1-g^2}{{\Bigl(1+g^2-2g\,(cc^*+ss^*+\zeta\zeta^*)\Bigr)}^{\frac{3}{2}}}.
\end{equation}
其中参数 $g$ 控制前向与后向散射的相对权重：$g=0$ 对应各向同性散射，$g\to 1$ 则对应高度前向集中的散射。尽管数值实验基于 H-G 相函数，算法本身仍可直接适配其他散射模型。

\subsection{数据集构造通用部分}\label{sec:dataset-construction}

训练集与测试集均由传统数值方法生成，主要采用扫掠法（sweeping method）~\cite{koch1991solution,zeyao2004parallel}。
...
\paragraph{角度求积点}
角度求积点选取为区间 $[-1,1]$ 上 $2N$ 阶标准勒让德多项式的正根。本文取 $N=3$，对应的求积点与权重列于表~\ref{tab:quadrature} ，其中，$\zeta_i$ 与 $\theta_i$ 分别为速度空间的求积点，$c_i$ 与 $s_i$ 为对应的余弦与正弦值，$w_i$ 为求积权重。总共 $24$ 个角度求积点覆盖了半球空间。
...
\begin{table}[!htp]
    ...
    \begin{tabular}{ccccc}
        ...
        \bottomrule
    \end{tabular}
    \bicaption{角度求积点及其权重。}{Angular quadrature points and weights of the quadrature set.}\label{tab:quadrature}
\end{table}
...
\subsection {DeepRTE 特有数据集与训练设置}

本节介绍 DeepRTE 数值实验中针对无源项模型的特有数据集构造与训练设置。相较于通用设定，DeepRTE 的数据集构造采用了特定的边界条件设计，以全面评估模型在不同物理情形下的表现。
...
\begin{equation}
    \begin{aligned}
        \sigma_{\br}     & \sim \mathcal{U}(0.005\sqrt{2},0.02\sqrt{2}), \\
        \sigma_{\bOmega} & \sim \mathcal{U}(0.005\sqrt{2},0.01\sqrt{2}).
    \end{aligned}
\end{equation}

数据集构造所用的超参数汇总于表~\ref{tab:dataset}。表中给出了计算区域、截面系数、离散参数、边界条件以及散射参数等设置。
\begin{table}[!htp]
    \centering
    \begin{tabular}{lp{0.3\textwidth}ll}
        ...
        \multirow{5}{*}{边界条件} & 光束空间中心坐标                 & $y_l', y_r', x_b', x_t'$ & 从网格点中随机采样                            \\
                              & \multirow{2}{*}{光束角度求积点} & $c_l', c_r', c_b', c_t'$ & \multirow{2}{*}{从角度求积点中随机采样}         \\
                              &                          & $s_l', s_r', s_b', s_t'$ &                                      \\
                              & 光束空间标准差                  & $\sigma_{\br}$           & $\sqrt{2}\,\mathcal{U}(0.005, 0.02)$ \\
                              & 光束角度标准差                  & $\sigma_{\bOmega}$       & $\sqrt{2}\,\mathcal{U}(0.005, 0.01)$ \\
        \midrule
        ...
        \bottomrule
    \end{tabular}
    \bicaption{数据集构造中使用的超参数汇总。}{Summary of hyperparameters for dataset construction.}\label{tab:dataset}
\end{table}
...
模型训练采用 Adam 优化器与余弦退火（cosine annealing）学习率策略。对每个散射区间，各生成 $800$ 个训练样本与 $200$ 个验证样本。训练在四张 NVIDIA RTX 4090 GPU 上进行，共训练 $5000$ 个 epoch。初始学习率设为 $1 \times 10^{-3}$，batch size 取 $8$，损失函数中使用的采样点数（见~\eqref{eq:collocation_points}）为 $128$。训练超参数如表~\ref{tab:training_details} 所示，表中列出了优化器、学习率策略与初值、数据规模以及硬件配置等信息。
...
\begin{table}[!htp]
    ...
    \begin{tabular}{lll}
        \toprule
        模块                    & 超参数                                     & 取值    \\ \midrule
        \multirow{7}{*}{衰减模块} & \texttt{num\_layer} $N_{\text{mlp}}$    & 4     \\
        ...
        \midrule
        \multirow{2}{*}{散射模块} & \texttt{num\_block} $N_{\ell}$          & $2$   \\
                              & \texttt{latent\_dim} $d_{\text{model}}$ & $16$  \\
        \bottomrule
    \end{tabular}
    \bicaption{DeepRTE 网络超参数。}{DeepRTE network hyperparameters.}\label{tab:parameters}
\end{table}
...
\begin{table}[!htp]
    ...
    \begin{tabular}{@{}cccccc@{}}
        \toprule
        模型                       & 参数量                    & 散射区间   & $g$ 取值范围   & MSE ($\times 10^{-10}$) & RMSPE (\%) \\ \midrule
        \multirow{3}{*}{DeepRTE} & \multirow{3}{*}{37954} & 近各向同性  & (0, 0.2)   & 5.630                   & 2.827      \\ \cmidrule(l){3-6}
                                 &                        & 中度各向异性 & (0.4, 0.6) & 5.453                   & 2.759      \\ \cmidrule(l){3-6}
                                 &                        & 强前向散射  & (0.7, 0.9) & 7.223                   & 3.181      \\ \bottomrule
    \end{tabular}
    \bicaption{DeepRTE 在三种散射区间上的精度指标。}{Accuracy validation of DeepRTE model across three distinct scattering regimes.}\label{tab:accuracy}
\end{table}
...
\subsection{线性性质验证}\label{sec:linearity_val}
...
\begin{equation}
    \begin{aligned}
        \|(\ANN_{\theta^*} - \mathcal{A}) I^{40}_{-,0.01}\|_{L^2}
         & = \Bigl\|\sum_{i=0}^{39} w_i(\ANN_{\theta^*} - \mathcal{A})
        \,\delta^{\sigma_{\br}}_{\{y_i\}}(y)\Bigr\|_{L^2}              \\
         & \leq \sum_{i=0}^{39} |w_i|\,
        \|(\ANN_{\theta^*} - \mathcal{A})
        \,\delta^{\sigma_{\br}}_{\{y_i\}}(y)\|_{L^2}                   \\
         & \approx 8.385\times 10^{-5}.
    \end{aligned}
\end{equation}
...
\begin{table}[!htp]
    ...
    \begin{tabular}{cccc}
        ...
        \bottomrule
    \end{tabular}
    \bicaption{DeepRTE 在不同网格分辨率与测试集上的表现。}{Performance of DeepRTE across different resolutions and test sets.}\label{tab:mesh_independence_multiple_testsets}
\end{table}
