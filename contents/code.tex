...
本章聚焦辐射输运方程神经算子求解方法的工程化落地，系统完成了核心算法的代码实现、性能优化与对比基线构建。首先介绍代码库的整体架构、开发环境与技术选型原则；其次阐述辐射输运求解场景的数据集构建与标准化预处理流程；随后分别详细说明 DeepRTE 与 EfficientRTE 两大模型的代码实现细节，涵盖网络设计、训练管线与核心算子优化，并通过伪代码呈现核心执行流程；最后为保障对比实验的公平性，完成了多 GPU 并行优化快速扫掠法与传统神经算子基线模型的工程实现，为所提方法提供了统一的精度参考与性能对比标尺。
...
本小节主要描述了数据集的构建，本文使用的数据集托管在\href{https://github.com/mazhengcn/rte-dataset}{rte-dataset数据集}，其开源地址为\url{https://github.com/mazhengcn/rte-dataset}，以python实现，整个代码过程包括数据的生成以及处理，可随时复现。

我们将每个训练样本的输入与输出统一组织为若干张量特征，见表~\ref{tab:rte_features}。
...
高效的数据集构建与预处理流程具有多重意义：一是标准化的数据划分与物理约束格式保证了物理信息的严格落地，为模型训练提供了清晰的多任务监督信号；二是基于Grain的流水线加载大幅提升了数据吞吐效率，尤其在大规模场景下可有效消除数据加载瓶颈，释放GPU计算潜力；三是序列化的数据存储与可配置的加载流程提升了实验的可复现性与灵活性，支持不同几何构型、参数场景下的快速切换与对比验证。

\section{DeepRTE 训练实现}

本节系统阐述DeepRTE模型的代码实现与训练流程，核心思想是通过神经网络显式建模辐射输运方程的格林函数解算子。整体实现遵循模块化、可扩展设计，将网络结构、核心物理算子、训练逻辑与并行策略解耦， 在实现上充分发挥JAX的自动微分、即时编译与并行计算优势。整个网络框架以及训练流程，复现脚本都可见我们发布的开源项目\href{https://github.com/mazhengcn/deeprte/tree/main}{DeepRTE}，
其地址为\url{https://github.com/mazhengcn/deeprte/tree/main}。
\subsection{整体训练流程}

训练流程围绕物理约束损失函数展开，核心步骤包括参数初始化、前向传播、损失计算、反向传播与参数更新。损失函数由边界条件损失与方程残差损失构成，分别约束模型在边界上的精度与在域内满足辐射输运方程的程度。

整体的训练流程可以参考算法~\ref{alg:training}，其输入为包含网络设定以及训练参数（如学习率、批次大小等）的配置对象，输出为训练完成后的模型参数。流程首先初始化随机数种子与模型参数，随后构造优化器与训练状态，并创建数据加载器与检查点管理器。进入训练循环后，对每个批次数据进行前向传播，计算物理约束损失，并通过自动微分求解梯度，最后更新模型参数并定期保存检查点。
...
DeepRTE的网络结构围绕格林函数解算子建模展开（具体理论与流程见章节~\ref{chap:deeprte}），通过衰减模块与散射模块模拟辐射输运物理过程。网络输入特征分为两类，分别用于衰减模块与散射模块计算：
...
本节系统阐述EfficientRTE模型的代码实现与训练流程（具体流程与理论见章节~\ref{chap:prte}），核心思想是通过基函数展开与编码器-解码器架构，将含源项辐射输运方程的高维积分求解转化为低维向量运算，基于JAX AI技术栈构建高效训练框架。整体实现复用DeepRTE的物理算子模块，新增编码器模块与线性映射层，在保持高精度的同时显著降低计算复杂度。整个网络框架以及训练流程，复现脚本都作为DeepRTE的一个分支发布，具体可见开源项目\href{https://github.com/mazhengcn/deeprte/tree/efficient-rte}{DeepRTE}分支efficient-rte，
其地址为\url{https://github.com/mazhengcn/deeprte/tree/efficient-rte}。

\subsection{整体训练流程}

训练流程分为编码器预训练、Gram矩阵计算与解码器训练三个阶段，核心围绕源项重构损失与辐射强度拟合损失展开。

伪代码~\ref{alg:training_efficientrte} 展示了EfficientRTE的完整训练流程。首先初始化随机数种子与模型参数，随后构造优化器与训练状态，并创建数据加载器与检查点管理器。进入训练循环后，第一阶段仅更新编码器参数以最小化源项重构损失；第二阶段计算Gram矩阵作为解码器训练的物理约束；第三阶段固定编码器参数，仅更新解码器参数以最小化辐射强度拟合损失。
...
\begin{algorithm}[htp!]
    \caption{格林函数推理}\label{alg:green_inference}
    \SetAlgoLined
...
扫掠法（Sweeping Method）是求解辐射输运方程的经典数值方法，其核心思想是沿特征线方向（由角度离散点确定）依次更新空间网格点的角通量，通过迭代收敛得到稳态解。该方法因物理直观性强、精度可控，被广泛用于生成辐射输运问题的参考解。然而，传统扫掠法主要针对CPU架构设计，存在两大瓶颈：一是空间扫掠过程中网格点的更新依赖于上游点；二是角度离散与空间离散的高维组合导致计算量巨大，且难以与基于GPU的神经网络模型在相同硬件平台上进行效率对比。

为解决上述问题，本文实现了一套多GPU并行优化的快速扫掠法，通过混合策略，结合高效的设备间通信和GPU的并行计算能力，实现辐射输运方程参考解的快速生成，为DeepRTE等神经网络模型提供公平的效率对比基准。这套扫掠法的GPU实现详见\href{https://github.com/mazhengcn/rte-dataset/tree/dev/src/rte_dataset/generator/python/sweeping}{rte-dataset}项目中的dev分支，具体地址为\url{https://github.com/mazhengcn/rte-dataset/tree/dev/src/rte_dataset/generator/python/sweeping}。

\subsection{主要思想与设计原则}

扫掠法的计算流程可拆解为三个核心步骤（传统数值算法理论见章节~\ref{sec:source-iteration}）：

\begin{itemize}
    \item \textbf{散射矩计算}：利用所有角度的角通量积分得到散射源项；
    \item \textbf{角度扫掠}：沿每个角度的特征线方向依次更新空间网格点；
    \item \textbf{收敛检查}：判断迭代是否达到稳态。
\end{itemize}
% （1）\textbf{散射矩计算}：利用所有角度的角通量积分得到散射源项；（2）\textbf{角度扫掠}：沿每个角度的特征线方向依次更新空间网格点；（3）\textbf{收敛检查}：判断迭代是否达到稳态。
其中，“散射矩计算”需要全局角度信息，而“角度扫掠”在不同角度之间相对独立，这为并行化提供了切入点。

本文的并行设计在全局上将角度离散点均匀分配到多个GPU设备，每个设备负责部分角度的角通量更新与散射矩计算，减少单设备内存压力并提升计算并行度；而在单个GPU内部，将空间网格点重新组织，依赖关系已满足的网格点可同时由多个GPU线程计算，挖掘空间维度的并行潜力；同时，通过“全收集（All-Gather）”通信原语，在散射矩计算前同步所有设备的角通量，既保证全局信息共享，又最小化通信开销。
...
散射矩计算需要所有角度的角通量信息，因此首先通过$\texttt{All-Gather}$通信，将每个设备的局部角通量$I^{d,[\ell]}_{i,j,k}$汇总为全局角通量$I^{[\ell]}_{i,j,k'}$（$k'$覆盖全部角度）。$\texttt{All-Gather}$是GPU集群中高效的集合通信操作，可快速完成数据同步，避免点对点通信的高延迟。
...
除上述流程外，本文的GPU并行实现还包含以下优化细节：空间网格点按行 / 列优先连续存储，确保 GPU 全局内存访问合并；对不同特征线方向可以启动多 GPU 线程并行执行；当前角度扫掠时预启动下一轮 \texttt {All-Gather} 通信，隐藏延迟。
...
关于神经算子(包括DeepRTE和MIO)的相关理论与实现细节，读者可参考章节~\ref{sec:neural-operator}，关于这个章节的代码实现，可见开源项目\href{https://github.com/mazhengcn/deeprte/tree/main}{DeepRTE}的MIO分支，具体地址为\url{https://github.com/mazhengcn/deeprte/tree/MIO}。以下重点介绍MIO在RTE求解中的具体实现以及特殊设计。
...
\begin{equation}
    I(\boldsymbol{r},\boldsymbol{\Omega}) \approx \text{MIO}_{\mathcal{L}}(\mu_t,\mu_s;q,I_-) := \left( \boldsymbol{b}_1 \odot \boldsymbol{b}_2 \odot \boldsymbol{b}_3 \odot \boldsymbol{b}_4 \right) \cdot \boldsymbol{t}(\boldsymbol{r},\boldsymbol{\Omega}).
\end{equation}

\section{本章总结}

本章围绕辐射输运方程神经算子求解的工程化落地、高效实现与并行优化展开，完整构建了从开发环境、数据构建、模型训练到数值基准加速的全流程技术体系。整体基于JAX技术栈实现，依托自动微分、XLA编译与原生并行原语，实现了代码模块化、训练高效化与部署标准化，为DeepRTE与EfficientRTE提供了可复现、可扩展、高性能的工程支撑。

在模型实现层面，DeepRTE严格遵循格林函数解算子物理结构，实现了包含相对位置编码、光学深度网络、衰减模块与散射模块的完整网络，并设计了训练流程；EfficientRTE在此基础上扩展编码器-解码器架构，通过三阶段训练流程学习源项基函数与格林函数系数，将高维积分转化为低维向量运算，显著提升计算效率。两套模型均实现了多GPU数据并行。

在数值基准与对比实现层面，本章提出了面向多GPU的扫掠法并行优化方案，通过角度域并行、空间变量并行与高效集合通信，提升传统数值解生成速度，同时为神经算子提供公平高效的对比基准；也完成了传统多输入算子MIO的基线实现，通过统一分支-主干网络结构，为后续验证对比DeepRTE物理先验架构奠定基础。
