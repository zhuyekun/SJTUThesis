...
为解决上述问题，本文实现了一套多GPU并行优化的快速扫掠法，通过混合策略，结合高效的设备间通信和GPU的并行计算能力，实现辐射输运方程参考解的快速生成，为DeepRTE等神经网络模型提供公平的效率对比基准。
...
关于神经算子(包括DeepRTE和MIO)的相关理论与实现细节，读者可参考章节~\ref{sec:neural-operator}，以下重点介绍MIO在RTE求解中的具体实现。
...
在基础架构层面，本章确立了以JAX+Flax+Optax+Orbax为核心的技术栈，通过uv依赖管理、Grain数据加载、静态图编译与模块化设计，保证了开发效率与运行效率的统一；数据集构建采用统一格式与无网格随机采样策略，实现了物理量的标准化表示与灵活加载，为模型训练提供了高质量、高吞吐的数据支撑。
