...
本节回顾文献中关于稳态RTE解存在唯一性的相关结果。首先给出更一般情形下（即 $\mut$ 和 $\mus$ 同时依赖于 $\br$ 和 $\bm{v}$）在 $L^p$ 空间（$1\leq p\leq\infty$）中的存在唯一性定理。
...
傅里叶神经算子（FNO）的设计灵感源于神经算子框架与傅里叶变换的结合。神经算子的核心是将函数到函数的映射分解为线性算子与非线性激活算子的交替组合，其结构与传统神经网络中线性层与激活函数的组合逻辑相似，但针对连续的函数输入，其线性变换被构造为积分算子。具体而言，对于一个输入函数 $ v(x) $，其线性映射输出 $ v'(x) $ 可表示为：
