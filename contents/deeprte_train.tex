...
需要说明的是，这两种方法都可以解稳态输运方程（可见~\ref{sec:preliminaries-solution-structure}），本文在此分别选取两种情形，旨在更清晰地展示两种方法各自的优势与适用场景。
...
上式中的两项分别对应于平滑误差和平面离散误差。通过适当选择 $\sigma$ 可在二者之间取得平衡：若 $\sigma$ 远小于粒子间最小距离，则近似函数仅在粒子 $\sigma$ 邻域内非零，会产生较大离散误差；反之，过大的 $\sigma$ 会带来显著的过度平滑误差。理论分析表明，常取 $\sigma\sim O(\sqrt{h})$ 以平衡两类误差。
% \todo[inline]{相关文献的reference待补充}
...
本节针对含体源项的稳态辐射输运方程，介绍EfficientRTE的训练框架。与DeepRTE类似，我们仍采用扫掠法生成参考数据，但在边界条件、源项构造、训练策略上进行针对性设计。整体流程分为含源项数据集构造，两阶段损失函数与训练和精度评估三部分。
...
此外，也可补充编码器重构精度评估（源项的MSE/RMSPE），用于验证基函数的表达能力。

\section{本章总结}

本章系统构建了辐射传输场景的两套完整训练框架，虽然在设计之初DeepRTE和EfficientRTE都可以处理稳态辐射输运方程，但是为了体现两种方法的特性，本章分别针对无源项稳态RTE设计了DeepRTE训练方法，针对含体源项稳态RTE设计了EfficientRTE训练方法，完成了从数据集构造、损失函数设计、优化策略到标准化评估体系的设计，为两类模型的性能验证与工程应用提供了可复现、可扩展的训练范式。

针对无源项稳态RTE求解，本章的核心在于提出了Delta函数训练数据集的构建方法。该方法深度利用RTE解算子关于入射边界条件的线性本质，通过高斯光滑子对边界Dirac源进行钝化处理，构造了一族近似Delta型的入射边界条件作为训练样本，其核心思想可类比于算子层面的预训练：模型仅需在Delta型边界的基础数据集上完成训练，即可依托解算子的线性结构，通过线性叠加自然推广到任意形式的一般边界条件，从理论上保证了模型的零样本外推能力。在此基础上，本章引入粒子近似方法与光滑子理论，严格推导了边界条件近似的误差界，并进一步证明了DeepRTE对任意边界条件的整体误差估计定理，为训练框架的收敛性与泛化性提供了坚实的数学支撑。在训练实现上，本章设计了基于相空间随机配点的均方误差损失函数，在保证优化精度的同时大幅降低了训练的计算成本；同时建立了以角度积分辐射强度为核心的评估体系，采用MSE与RMSPE作为统一精度指标，既贴合辐射传输领域的工程应用习惯，也为不同模型、不同算例之间的性能对比提供了参照。

针对含体源项稳态RTE求解，本章在DeepRTE训练框架的基础上，完成了针对性的适配与优化，构建了EfficientRTE的两阶段训练体系。数据集构造上，采用齐次入射边界条件以聚焦于体源项的求解，通过频域功率谱方法生成二维高斯随机场作为体源项，保证了训练样本的多样性与空间分布的平滑可控性，且数据格式与前面DeepRTE的数据生成完全兼容，具备良好的工程复用性。训练策略上，基于EfficientRTE的编码器-解码器架构，设计了“编码器预训练-解码器微调”的两阶段训练流程：第一阶段通过源项重构损失预训练编码器，学习具备强表达能力的源域基函数并预计算Gram矩阵，实现体源项的低维降维；第二阶段固定编码器参数与Gram矩阵，通过辐射强度拟合损失训练解码器，深度复用DeepRTE的衰减模块与散射模块以保留物理归纳偏置，仅通过末端线性层适配基函数维度，完成格林函数观测域系数的精准学习。该训练框架同时支持端到端联合训练模式，可根据实际需求灵活调整训练策略，在保持与DeepRTE相当求解精度的同时，从根本上解决了含源项场景下高维相空间积分的计算瓶颈。

两套训练框架既保持了设计逻辑的统一性，又形成了适用场景的互补性：二者均严格遵循RTE的线性物理本质，以传统数值解作为训练基准，采用随机配点策略平衡训练效率与精度；同时分别针对无源项场景的边界条件泛化需求、含源项场景的高维计算效率需求做了专属优化，共同构成了稳态辐射传输方程神经算子求解的完整过程。