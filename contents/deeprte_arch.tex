...
\todo[inline]{~\cite{adams2002fast}}
形式一致。进一步地，根据
\todo[inline]{~\cite{egger2014lp}}
的结论可知：在定理~\ref{thm:existence-uniqueness-lp} 的假设条件下，对任意 $1\leq p\leq \infty$，有
\begin{equation}
    \rho_p(\cL\cS) := \lim_{n\to\infty}\sqrt[n]{\| (\cL\cS)^n \|_{L^p(D\times V;\tau^{-1})}} \leq 1 - e^{-C_p} < 1.
\end{equation}
这表明不动点算子 $\cL\cS$ 的谱半径被一致地界定在 1 之下，从而保证迭代收敛到 RTE 的唯一解。因此，解算子可展开为级数形式：
...
\begin{figure}[htp!]
    ...
    \bicaption{散射块。表示模拟算子 $\cS$ 作用一次的结果。包括积分的加权求和、线性投影和激活。}{Scattering module. Stacked residual blocks with physics-informed $\cS$-approximation layers, employing tanh-activated operator transforms and adaptive layer normalization for stabilized learning.}\label{fig:scattering-block}
\end{figure}
...
\begin{equation}
    \bm{G}^{0} = \mathrm{Concat}\left(\bm{G}^{\text{NN}}, \mathbf{W}^\ell\right), \quad
    \bm{G}^{\text{NN}} =
    \begin{pmatrix}
